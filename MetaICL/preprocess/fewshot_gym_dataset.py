...
## Same code as MetaICL/preprocess/fewshot_gym_dataset.py just changed parse_args() to parse_known_args()
...
class FewshotGymDataset():
    ...
    def save(self, path, k, seed, k_shot_train, k_shot_dev, k_shot_test):
        ...
        if do_train and use_instruct:
            ...
            if path:
                identifier = self.hf_identifier.split('/')[-1]
                os.makedirs(os.path.join(path, identifier), exist_ok=True)
                prefix = os.path.join(path, identifier,
                                      "{}_{}_{}".format(identifier, k, seed))
